...
public class HandChecker {
	...
	public ArrayList<ArrayList<Player>> determineWinner() {
		...
		for(int i = 0; i < numPlayers; i++) {
			if(handRankings[i] == currBestHandVal) {
				...
				//There is a tie, rank the ties then insert based in the rank of the ties
					...
					handValues = tieChecker(tiedHands, currBestHandVal);
					int currTieVal = 1;
	...
	public int[] tieChecker(ArrayList<Hand> tiedHands, int tieNumber) {
		int numOfTiedHands = tiedHands.size();
		ArrayList<Card> communityCards = new ArrayList<Card>();
		int[] handValues = new int[numOfTiedHands];
		
		communityCards.add(gameState.getRiver());
		communityCards.add(gameState.getTurn());
		communityCards.add(gameState.getFlop().get(0));
		communityCards.add(gameState.getFlop().get(1));
		communityCards.add(gameState.getFlop().get(2));
